document.addEventListener("DOMContentLoaded", () => {
  ...
  if (rows.length === 0) {
    ...
    rows.sort((a,b)=> (b.completedAt||"").localeCompare(a.completedAt||"")).forEach(r => {
      const tr = document.createElement("tr");
      tr.innerHTML = `<td>${r.student||"-"}</td><td>${r.title||"-"}</td><td>${r.videoId||"-"}</td><td>${r.completedAt?new Date(r.completedAt).toLocaleString():"-"}</td>`;
      body.appendChild(tr);
    });
  }
